...
assert.equal(pkg.scripts?.["cf:preview"],"npm run cf:build && wrangler versions upload");
assert.equal(pkg.scripts?.["cf:deploy"],"npm run cf:build && wrangler deploy");
assert.match(pkg.scripts?.["test:build-gate"]||"",/tests\/cloudflare-build-gate-contract\.mjs/);
assert.equal(wrangler.name,"intelligence-worker");
