...
const x=j?.results?.find(v=>v.provider==="google_knowledge_graph");assert.ok(x,"missing google_knowledge_graph");assert.equal(x.status,"PASS",`google_knowledge_graph failed: ${x.error||x.http}`);
console.log(JSON.stringify({ok:true,stage:"google-knowledge-graph"}));
