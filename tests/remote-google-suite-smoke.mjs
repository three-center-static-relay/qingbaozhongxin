...
for(const p of ["bigquery","earthengine"]){const x=j?.results?.find(v=>v.provider===p);assert.ok(x,`missing ${p}`);assert.equal(x.status,"PASS",`${p} failed/status=${x.status}: ${x.error||x.http}`)}
console.log(JSON.stringify({ok:true,stage:"google-cloud-data"}));
