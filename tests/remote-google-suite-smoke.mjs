...
const x=j?.results?.find(v=>v.provider==="bigquery");assert.ok(x,"missing bigquery");assert.equal(x.configured,true,"bigquery cloud credentials not visible");
console.log(JSON.stringify({ok:true,stage:"bigquery-configured",status:x.status}));
