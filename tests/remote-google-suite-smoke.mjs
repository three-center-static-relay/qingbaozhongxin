...
const x=j?.results?.find(v=>v.provider==="google_factcheck");assert.ok(x,"missing google_factcheck");assert.equal(x.status,"PASS",`google_factcheck failed: ${x.error||x.http}`);
console.log(JSON.stringify({ok:true,stage:"google-factcheck"}));
