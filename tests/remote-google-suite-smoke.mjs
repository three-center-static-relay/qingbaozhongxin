...
for(const p of ["google_knowledge_graph","google_crux","google_pagespeed"]){const x=j?.results?.find(v=>v.provider===p);assert.ok(x,`missing ${p}`);assert.equal(x.status,"PASS",`${p} failed: ${x.error||x.http}`)}
console.log(JSON.stringify({ok:true,stage:"google-knowledge-webintel"}));
