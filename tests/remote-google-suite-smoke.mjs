...
const x=j?.results?.find(v=>v.provider==="earthengine");assert.ok(x,"missing earthengine");assert.equal(x.status,"PASS",`earthengine failed/status=${x.status}: ${x.error||x.http}`);
console.log(JSON.stringify({ok:true,stage:"earthengine"}));
