...
for(const [provider,operation,args] of cases){
  const task_id=`fresh-rare-genetics-20260817-${Date.now()}-${++i}`;
...
console.log(JSON.stringify({ok:true,suite:"fresh-rare-genetics-upstreams",providers:cases.map(x=>x[0]),count:cases.length,real_upstream:true,patient_data:false}));
