...
try{
  ...
  assert.ok(out.place.aliases.includes("福州宝龙广场"));
  assert.ok(out.plan.length>=9);
  const providers=new Set(out.plan.map(x=>x.provider));
  for(const p of ["tencent_maps","baidu_maps","geonames","mobilitydatabase","network-intelligence-collection","compute-center"])assert.ok(providers.has(p),`missing ${p}`);
  const tencent=out.plan.find(x=>x.provider==="tencent_maps"&&x.operation==="place_text");assert.equal(tencent.required,false);assert.match(tencent.controller_policy,/aliases-serially/);
  ...
  console.log(JSON.stringify({ok:true,suite:"geospatial-commercial-network-collaboration",case:"fuzhou-baolong-city-plaza",mode:out.mode,plan_stages:out.plan.length,compute_handoff:true,observed_lbs:false}));
