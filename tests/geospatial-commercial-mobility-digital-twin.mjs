...
for(const tool of ["activitysim","aequilibrae","conveyal_r5","eclipse_sumo","popcorn_population","ohsome_api"])assert.ok(COMMERCIAL_MOBILITY_DIGITAL_TWIN.open_source_models[tool],`missing ${tool}`);
for(const f of ["population_nowcast","activity_chain_demand","modelled_od_demand","modelled_dwell_proxy","temporal_accessibility","observed_parking_occupancy","poi_turnover"])assert.ok(COMMERCIAL_MOBILITY_DIGITAL_TWIN.normalized_feature_outputs.includes(f),`missing output ${f}`);
...
assert.equal(registry.accepted_open_source_tools.length,6);
...
for(const x of ["activitysim_BSD_3_clause","aequilibrae_MIT_attribution","conveyal_r5_MIT","eclipse_sumo_EPL_2_0","popcorn_population_Apache_2_0","ohsome_api_OSM_history_ODbL","commercial-spatial-knowledge-graph-v1"])assert.ok(shared.includes(x),`domain missing ${x}`);
for(const f of ["population_nowcast","modelled_od_demand","modelled_dwell_proxy","observed_public_transport_activity","observed_parking_occupancy","commercial_entity_graph"])assert.ok(GEOSPATIAL_COMMERCIAL_DOMAIN.feature_layers.includes(f),`domain missing feature ${f}`);
