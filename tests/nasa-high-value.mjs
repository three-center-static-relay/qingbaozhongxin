...
assert.ok(Object.keys(NASA_HIGH_VALUE).length>=10);
for(const id of ["merra2","gpm_imerg","smap","ecostress","gedi","swot","grace_fo","tempo","sedac","black_marble","modis","viirs","landsat"])assert.ok(NASA_HIGH_VALUE[id],`missing NASA high-value preset ${id}`);
...
try{
  globalThis.fetch=async(url,init={})=>{const u=String(url);seen.push({u,init});
    ...
    if(u.includes("WMTSCapabilities.xml"))return new Response('<Capabilities xmlns:ows="http://www.opengis.net/ows/1.1"><ows:Identifier>MODIS_Terra_Test</ows:Identifier><ows:Identifier>VIIRS_Test</ows:Identifier></Capabilities>',{status:200,headers:{"content-type":"application/xml"}});
  ...
  const gibs=await run23("nasa_gibs","layers",{query:"modis",limit:10},{});assert.deepEqual(gibs.items,["MODIS_Terra_Test"]);
