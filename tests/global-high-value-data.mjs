...
for(const [provider,secret] of Object.entries(keySources)){
  ...
  if(entry.adapter!=="catalog-only")assert.ok(Array.isArray(OPERATIONS[provider])&&OPERATIONS[provider].length>0,`graduated live key provider has no approved operations: ${provider}`);
}

for(const provider of ["abs_australia","destatis_genesis","insee_france","istat_italy","cbs_netherlands","scb_sweden","ssb_norway","statfin_finland","statbank_denmark","gleif_lei","bank_canada_valet","us_census_api","us_bls_api","us_treasury_fiscaldata","faa_aerodata","noaa_accessais","copernicus_marine","emodnet","argo_gdac","gebco","world_bank_pink_sheet","imf_commodity_prices","jodi_oil_gas","nih_reporter","cdc_open_data","cms_data","usaspending","ted_eu_procurement","usgs_earthquake","gdacs","reliefweb","nomad_materials","aflowlib"]){assert.ok(CATALOG[provider],`missing global high-value source: ${provider}`)}
