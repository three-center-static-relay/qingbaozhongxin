...
try{
  globalThis.fetch=async(url,init={})=>{
    ...
    if(s.includes("overpass-api.de"))return new Response(JSON.stringify({version:0.6,osm3s:{timestamp_osm_base:"2026-08-16T00:00:00Z"},elements:[{type:"node",id:1,lat:26.07,lon:119.29,tags:{shop:"supermarket"}}]}),{status:200,headers:{"content-type":"application/json"}});
    if(s.includes("fi.ee.tsinghua.edu.cn/worldmove/data"))return new Response('<html><a href="/worldmove/files/fuzhou.zip">Fuzhou</a></html>',{status:200,headers:{"content-type":"text/html"}});
  ...
  assert.equal(seen[1].url,"https://overpass-api.de/api/interpreter");
  ...
  assert.ok(wm.links.some(x=>x.includes("fuzhou.zip")));
  assert.equal(seen[2].url,"https://fi.ee.tsinghua.edu.cn/worldmove/data");
...
console.log(JSON.stringify({ok:true,suite:"free-commercial-spatial-stack",h3_local:true,ors_free_key_fail_closed:true,overpass_fixed_query:true,worldmove_fixed_official_source:true,baidu_free_traffic_only:true,no_paid_autoupgrade:true}));
