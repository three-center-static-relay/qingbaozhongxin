...
for(const p of ["h3","openrouteservice","worldmove","baidu_maps"]){assert.ok(CATALOG[p],`missing ${p}`);assert.equal(CATALOG[p].arbitrary_url,false)}
...
console.log(JSON.stringify({ok:true,suite:"free-commercial-spatial-stack",h3_local:true,ors_free_key_fail_closed:true,overpass_removed_after_cloudflare_e2e_failure:true,worldmove_figshare_official_archive:true,baidu_free_traffic_only:true,no_paid_autoupgrade:true}));
