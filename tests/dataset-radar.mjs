...
assert.equal(meta.raw_dataset_mirror,false);
assert.equal(meta.raw_notebook_copy,false);
assert.equal(meta.arbitrary_url,false);
assert.equal(meta.max_candidates,100);
assert.ok(meta.sources.length>=11);
assert.equal(meta.sources.find(x=>x.id==="kaggle_datasets")?.configured,true);
assert.equal(meta.sources.find(x=>x.id==="kaggle_notebooks")?.configured,true);
assert.equal(meta.sources.find(x=>x.id==="modelscope")?.configured,false);
assert.equal(meta.sources.find(x=>x.id==="modelscope")?.required_secret,"MODELSCOPE_API_TOKEN");

assert.equal(__test.chinaMatch("福建福州商业房地产人口数据"),true);
assert.equal(__test.chinaMatch("generic unrelated benchmark"),false);
...
assert.ok(scoreCandidate(cn)>scoreCandidate(generic));
assert.equal(scoreCandidate({...cn,title:"credential leak password dump"}),-100);
...
assert.equal(merged.length,1);assert.equal(merged[0].score,80);
assert.ok(String(__test.queryFor(Date.UTC(2026,7,15),0)).length>0);

const registry=JSON.parse(readFileSync(new URL("../data-assets/dataset-radar-sources.json",import.meta.url),"utf8"));
assert.equal(registry.policy.metadata_only_recurring,true);
assert.equal(registry.policy.raw_dataset_mirror,false);
assert.equal(registry.policy.raw_notebook_copy,false);
assert.equal(registry.policy.task_bound_download_only,true);
assert.ok(registry.sources.length>=30);
