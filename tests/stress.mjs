...
async function waves(total,width,fn,label){const out=[];for(let base=0;base<total;base+=width){const part=await within(Promise.all(Array.from({length:Math.min(width,total-base)},(_,i)=>fn(base+i))),7000,`${label}-${base}`);out.push(...part)}return out}
...
try{
  ...
  await reset();const burst=await waves(320,64,i=>post("/v1/run",{task_id:`rate-${i}`,provider:"no_such_provider",operation:"x"}),"rate-wave");assert.equal(burst.filter(x=>x.status===429&&x.body?.error==="RATE_LIMITED").length,20);
  const health=await waves(256,64,()=>server.fetch("/health"),"health-wave");assert.equal(health.filter(r=>r.status===200).length,256);
  console.log(JSON.stringify({ok:true,suite:"intelligence-stress",max_parallel:64,concurrency_contenders:64,duplicate_contenders:64,rate_total:320,health_total:256,tests:["single-lock","duplicate-id","cancel-release","failure-release","policy-deny","body-limit","rate-limit","read-burst"]}));
