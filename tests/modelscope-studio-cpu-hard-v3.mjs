...
try{
  ...
  assert.equal(r.status,200,`ModelScope Studio CPU v3 E2E HTTP ${r.status}: ${JSON.stringify(body)}`);
  ...
  assert.ok(String(h?.name||"").length>0,"Hardware name required");
  const receipt=body?.runtime_receipt||{};
  assert.equal(receipt?.ok,true);
  assert.equal(receipt?.revision,"studio-cpu-runtime-v3-20260817");
  ...
  console.log(JSON.stringify({ok:true,suite:"modelscope-studio-cpu-hard-v3-production-e2e",stage:body.stage,hardware:{name:h.name,label:h.label||null,cpu:h.cpu,memory_gb:h.memory_gb,free:true},runtime_receipt:{revision:receipt.revision,cpu_effective:receipt.cpu_effective,memory_gb_effective:receipt.memory_gb_effective,square_sum_correct:true,result_digest:receipt.result_digest,python:receipt.python,numpy:receipt.numpy,torch:receipt.torch},free_only:true,paid_fallback:false,secrets_redacted:true}));
