...
for(const p of ["exa","tavily","firecrawl"]){
...
assert.deepEqual(OPERATIONS.jina,["read"]);
assert.deepEqual(ALL_OPERATIONS.jina,["read"]);
assert.ok(CATALOG.jina,"jina must be aggregated into the live catalog");
assert.equal(CATALOG.jina.access,"public");
assert.equal(CATALOG.jina.adapter,"jina.reader");
assert.equal(CATALOG.jina.endpoint,"https://r.jina.ai");
assert.equal(CATALOG.jina.search,false);
assert.equal(CATALOG.jina.public_url_only,true);
assert.equal(CATALOG.jina.write,false);
assert.equal(WEB_SEARCH_CATALOG.jina.secrets,undefined);
...
try{
  globalThis.fetch=async(url,init={})=>{const u=new URL(String(url));const headers=Object.fromEntries(Object.entries(init.headers||{}).map(([k,v])=>[String(k).toLowerCase(),String(v)]));let body={};try{body=JSON.parse(init.body||"{}") }catch{}calls.push({u,headers,body,method:init.method||"GET"});
    ...
    if(u.hostname==="r.jina.ai")return textResponse("# Example\n\nReader content");
    throw new Error(`unexpected host ${u.hostname}`)
  };
  const env={EXA_API_KEY:"exa-key",TAVILY_API_KEY:"tv-key",FIRECRAWL_API_KEY:"fc-key",JINA_API_KEY:"must-not-be-used"};
  ...
  const jina=await runAll("jina","read",{url:"https://example.com/article?a=1"},env);assert.match(jina.content,/Reader content/);assert.equal(jina.free_mode,true);
  ...
  assert.equal(calls[3].u.origin,"https://r.jina.ai");assert.ok(calls[3].u.href.startsWith("https://r.jina.ai/https://example.com/article"));assert.equal(calls[3].headers.authorization,undefined);assert.equal(calls[3].headers["x-retain-images"],"none");
  ...
  await assert.rejects(()=>runAdapter("jina","search",{query:"x"},{}),/ADAPTER_OPERATION_NOT_APPROVED/);
  await assert.rejects(()=>runAdapter("jina","read",{url:"http://127.0.0.1/private"},{}),/PRIVATE_URL_NOT_ALLOWED/);
  await assert.rejects(()=>runAdapter("jina","read",{url:"file:///etc/passwd"},{}),/URL_SCHEME_NOT_ALLOWED/);
  console.log(JSON.stringify({ok:true,suite:"web-search-providers",search_providers:["exa","tavily","firecrawl"],jina_operation:"read-only",jina_search_disabled:true,jina_no_key_basic_reader:true,jina_public_url_only:true,catalog_aggregated:true,aggregate_router:true,write:false,bounded:true}));
