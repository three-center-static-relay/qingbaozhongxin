...
const TIMEOUT_MS=60000,PROBE_REV=9;
...
assert.equal(x.j?.error,"UPSTREAM_AUTH_FAILED",`unexpected classification http=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification:"UPSTREAM_AUTH_FAILED",base,probe_rev:PROBE_REV}));
