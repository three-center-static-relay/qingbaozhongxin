...
const TIMEOUT_MS=60000,PROBE_REV=7;
...
assert.equal(x.j?.error,"ARG_REQUIRED:project_id",`unexpected classification http=${x.r.status} error=${x.j?.error||"none"} upstream=${x.j?.details?.http_status||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification:"MISSING_PROJECT_ID",base,probe_rev:PROBE_REV}));
