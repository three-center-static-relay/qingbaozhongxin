...
const TIMEOUT_MS=60000,PROBE_REV=14;
...
assert.equal([400,403,409,429].includes(x.r.status),true,`not 4xx-class status=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification_group:"HTTP_4XX",status:x.r.status,base,probe_rev:PROBE_REV}));
