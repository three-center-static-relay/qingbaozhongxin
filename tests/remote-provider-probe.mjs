...
const TIMEOUT_MS=60000,PROBE_REV=15;
...
assert.equal([500,503,504].includes(x.r.status),true,`outside service class status=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification_group:"HTTP_5XX_SERVICE",status:x.r.status,base,probe_rev:PROBE_REV}));
