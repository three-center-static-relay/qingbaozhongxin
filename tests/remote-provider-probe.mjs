...
const TIMEOUT_MS=60000,PROBE_REV=12;
...
const group=new Set(["POLICY_DENIED_PUBLIC_DATA_ONLY","POLICY_DENIED_READ_ONLY_QUERY","BUSY","RATE_LIMITED","UPSTREAM_TIMEOUT","UPSTREAM_RESPONSE_TOO_LARGE","UPSTREAM_BAD_JSON","LOCK_BACKEND_UNAVAILABLE","ADAPTER_NOT_IMPLEMENTED","ADAPTER_OPERATION_NOT_APPROVED","INVALID_COUNTRY_CODE"]);
assert.equal(group.has(x.j?.error),true,`outside diagnostic group http=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification_group:"POLICY_RUNTIME_OR_TRANSPORT",base,probe_rev:PROBE_REV}));
