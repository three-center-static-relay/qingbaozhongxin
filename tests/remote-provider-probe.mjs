...
const TIMEOUT_MS=60000,PROBE_REV=13;
...
assert.equal(x.r.status,502,`unexpected status=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification:"HTTP_502_UNNORMALIZED_RUNTIME_ERROR",base,probe_rev:PROBE_REV}));
