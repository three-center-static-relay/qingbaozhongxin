...
const TIMEOUT_MS=90000,PROBE_REV=4;
...
assert.equal(x.r.ok&&x.j?.ok===true,true,`${provider} E2E failed http=${x.r.status} error=${x.j?.error||"none"}`);assert.ok(x.j?.result?.data,`${provider} returned empty page info`);
