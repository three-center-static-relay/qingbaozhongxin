...
const TIMEOUT_MS=60000,PROBE_REV=8;
...
assert.equal(x.j?.error,"UPSTREAM_HTTP_ERROR",`unexpected classification http=${x.r.status} error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification:"UPSTREAM_HTTP_ERROR",upstream_http:x.j?.details?.http_status||null,base,probe_rev:PROBE_REV}));
