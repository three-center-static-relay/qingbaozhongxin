...
try{
  ...
  assert.ok(Number(body?.provider_count)>0,"Router must return at least one provider record");
  ...
  assert.equal(body?.free_status_verified,true,"At least one provider must explicitly expose boolean is_free; zero pricing is not proof");
  ...
  assert.ok(body.providers.some(p=>p?.is_free===true),"Normalized providers must contain explicit is_free=true");
