...
async function ready(provider){const {r,j}=await request(`${base}/v1/provider/${provider}/readiness`);assert.equal(r.status,200,`${provider}: readiness route failed`);assert.equal(j?.configured,true,`${provider}: runtime configuration missing`);return j}
async function run(provider,operation,args){const task_id=`smoke-${provider}-${Date.now()}-${crypto.randomUUID()}`;const x=await request(`${base}/v1/run`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({task_id,provider,operation,timeout_seconds:70,args})});assert.equal(x.r.ok&&x.j?.ok===true,true,`${provider}.${operation} failed http=${x.r.status} error=${x.j?.error||"none"}`);assert.ok(x.j?.result,`${provider}.${operation}: empty result envelope`);return x.j}
