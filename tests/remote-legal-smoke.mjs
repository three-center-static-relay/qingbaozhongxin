...
assert.equal(x.r.ok&&x.j?.ok===true,false,"Google Trends unexpectedly passed");assert.equal(x.r.status>=500,true,`Google Trends failure is client/policy class http=${x.r.status} error=${x.j?.error||"none"}`);console.log(JSON.stringify({ok:true,classification:"SERVER_CLASS"}));
