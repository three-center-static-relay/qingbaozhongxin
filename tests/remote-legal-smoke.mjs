...
assert.equal(x.r.ok&&x.j?.ok===true,false,"Google Trends unexpectedly passed");assert.equal(x.r.status,400,`not 400; http=${x.r.status} error=${x.j?.error||"none"}`);console.log(JSON.stringify({ok:true,classification:"HTTP_400"}));
