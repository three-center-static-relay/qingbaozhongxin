...
assert.equal(x.r.ok&&x.j?.ok===true,false,"Google Trends unexpectedly passed");const s=["POLICY_DENIED_PUBLIC_DATA_ONLY","UPSTREAM_TIMEOUT","RATE_LIMITED","BUSY","UPSTREAM_RESPONSE_TOO_LARGE","UPSTREAM_BAD_JSON","POLICY_DENIED"];assert.equal(s.includes(x.j?.error),true,`not classified set; error=${x.j?.error||"none"}`);
console.log(JSON.stringify({ok:true,classification:"POLICY_OR_RUNTIME_CLASS"}));
