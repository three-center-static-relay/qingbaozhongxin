...
for(const op of ["place_text","place_around","place_polygon","place_detail","district","driving","walking","transit","distance"]){assert.ok(OPERATIONS.amap.includes(op),`missing amap ${op}`)}

const oldFetch=globalThis.fetch;
const calls=[];
try{
  globalThis.fetch=async(url,init={})=>{const u=new URL(String(url));calls.push({u,init});return new Response(JSON.stringify({status:"1",info:"OK",infocode:"10000",count:"0",pois:[],districts:[],route:{paths:[],transits:[]}}),{status:200,headers:{"content-type":"application/json"}})};
  const env={AMAP_API_KEY:"test-amap-key"};

  const around=await runAdapter("amap","place_around",{location:"119.2965,26.0745",radius:8000,keywords:"购物中心",types:["060100","120100"],limit:25,region:"福州市",city_limit:true,sortrule:"weight"},env);
  let u=calls.at(-1).u;
  assert.equal(around.commercial_fields,true);
  ...
  assert.equal(u.searchParams.get("show_fields"),"children,business,indoor,navi");

  await runAdapter("amap","place_polygon",{polygon:["119.28,26.06","119.32,26.06","119.32,26.09","119.28,26.09","119.28,26.06"],types:"060100|120100",show_fields:["business","navi"],limit:20},env);
  u=calls.at(-1).u;
  ...
  assert.equal(u.searchParams.get("types"),"060100|120100");
  assert.equal(u.searchParams.get("show_fields"),"business,navi");

  await runAdapter("amap","place_detail",{id:["B0FFFTEST1","B0FFFTEST2"],show_fields:"business,indoor,navi"},env);
  u=calls.at(-1).u;
  assert.equal(u.pathname,"/v5/place/detail");
  assert.equal(u.searchParams.get("id"),"B0FFFTEST1|B0FFFTEST2");
  assert.equal(u.searchParams.get("show_fields"),"business,indoor,navi");

  await runAdapter("amap","district",{keywords:"福州市",subdistrict:1,extensions:"all"},env);
  u=calls.at(-1).u;
  assert.equal(u.pathname,"/v3/config/district");
  assert.equal(u.searchParams.get("keywords"),"福州市");
  assert.equal(u.searchParams.get("subdistrict"),"1");
  ...
  await assert.rejects(()=>runAdapter("amap","place_text",{keywords:"商场",show_fields:"business,secret"},env),/INVALID_SHOW_FIELDS/);
  ...
  console.log(JSON.stringify({ok:true,suite:"china-commercial-amap",poi_around:true,poi_polygon:true,poi_detail:true,commercial_business_fields:true,indoor_hierarchy:true,entrance_exit_navi:true,district_boundary:true,transit:true,route_cost_duration_fields:true,bounded:true}));
