...
try{
  ...
  globalThis.fetch=async(url,init={})=>{const body=JSON.parse(init.body||"{}");pkCalls.push({url:String(url),headers:init.headers||{},body});
    if(body.method==="initialize")return json({jsonrpc:"2.0",id:1,result:{protocolVersion:"2025-03-26",serverInfo:{name:"pkulaw-test",version:"1"}}},200,{"mcp-session-id":"session-1"});
    if(body.method==="notifications/initialized")return new Response("",{status:202});
    if(body.method==="tools/list")return json({jsonrpc:"2.0",id:2,result:{tools:[{name:"get_law_list",description:"test",inputSchema:{type:"object",properties:{title:{type:"string"}}}}]}});
    if(body.method==="tools/call"&&body.jsonrpc)return json({message:"Unclassified Authentication Failure"},401);
    if(body.method==="tools/call"&&!body.jsonrpc)return json({result:{content:[{type:"text",text:"劳动合同法测试结果"}]}});
  ...
  assert.equal(pk.compatibility_fallback_used,true);assert.equal(pk.data?.content?.[0]?.text,"劳动合同法测试结果");assert.equal(pkCalls.length,5);
  const sessionCall=pkCalls[3],fallbackCall=pkCalls[4];assert.equal(sessionCall.headers["mcp-session-id"],"session-1");assert.equal(fallbackCall.headers["mcp-session-id"],undefined);assert.equal(fallbackCall.headers["mcp-protocol-version"],undefined);assert.equal(fallbackCall.body.jsonrpc,undefined);assert.equal(fallbackCall.body.method,"tools/call");
  ...
  console.log(JSON.stringify({ok:true,suite:"runtime-repairs",pkulaw_401_official_fallback:true,google_provider_specific_keys:true,pagespeed_timeout_ms:35000,pagespeed_optional_key_fallback:true}));
