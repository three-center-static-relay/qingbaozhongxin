...
const TIMEOUT_MS=90000,PROBE_REV=2;
...
const base=await findBase(),provider="pkulaw";const rd=await request(`${base}/v1/provider/${provider}/readiness`);assert.equal(rd.r.status,200);assert.equal(rd.j?.configured,true,`${provider} readiness false`);
...
assert.equal(x.j?.error,"UPSTREAM_HTTP_ERROR",`unexpected error status=${x.r.status} error=${x.j?.error||"none"}`);assert.equal([401,403].includes(Number(x.j?.details?.http_status)),true,`not auth/permission failure upstream=${x.j?.details?.http_status||"none"}`);
console.log(JSON.stringify({ok:true,provider,classification:"BROWSERFABRIC_AUTH_OR_PERMISSION_FAILURE",upstream_http:x.j.details.http_status,probe_rev:PROBE_REV,base}));
