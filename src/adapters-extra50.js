...
async function zenodoSearch(args,env){
  const token=String(env.ZENODO_TOKEN||"").trim();
  if(!token)err("UPSTREAM_AUTH_FAILED",503,{provider:"zenodo",required_secrets:["ZENODO_TOKEN"]});
  const q=required(args.query,"query",500),limit=clamp(args.limit,1,25,10),page=clamp(args.page,1,100,1),sort=["bestmatch","mostrecent"].includes(args.sort)?args.sort:"bestmatch",u=new URL("https://zenodo.org/api/records/");
  u.searchParams.set("q",q);u.searchParams.set("size",String(limit));u.searchParams.set("page",String(page));u.searchParams.set("sort",sort);
  const r=await fetchBounded(u,{headers:{authorization:`Bearer ${token}`}}),body=r.body;
  const items=Array.isArray(body)?body:Array.isArray(body?.hits?.hits)?body.hits.hits:Array.isArray(body?.hits)?body.hits:[];
