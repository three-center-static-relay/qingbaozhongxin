...
function mapBound(v){const s=text(v,100),p=s.split(",").map(Number);if(p.length!==4||!p.every(Number.isFinite)||p[0]<-180||p[2]>180||p[1]<-90||p[3]>90||p[0]>=p[2]||p[1]>=p[3])throw Object.assign(new Error("INVALID_MAP_BOUND"),{status:400});return p.join(",")}
...
function tiandituKey(env){return env.TIANDITU_TK||env.TIANDITU_API_KEY||""}
...
export const OPERATIONS={baidu_maps:["geocode","reverse_geocode"],tencent_maps:["geocode","reverse_geocode","place_nearby"],tianditu:["search"]};
export async function runAdapter(provider,operation,args,env){
  ...
  if(provider==="tianditu"){
    const tk=tiandituKey(env);if(!tk)throw Object.assign(new Error("UPSTREAM_AUTH_FAILED"),{status:503});const postStr={keyWord:text(required(args,"keyword"),100),level:clamp(args.level,1,18,12),mapBound:mapBound(required(args,"map_bound")),queryType:[1,7].includes(Number(args.query_type))?Number(args.query_type):1,start:0,count:clamp(args.limit,1,50,10)};if(args.specify)postStr.specify=text(args.specify,30);const u=new URL("https://api.tianditu.gov.cn/v2/search");u.searchParams.set("postStr",JSON.stringify(postStr));u.searchParams.set("type","query");u.searchParams.set("tk",String(tk));const body=await getJson(u);return{provider,operation,data:body}
  }
