...
const WORLDMOVE_DATA="https://fi.ee.tsinghua.edu.cn/worldmove/data";
...
async function overpassRun(args){const[lon,lat]=lonLat(args?.location),radius=clamp(args?.radius,50,1500,500),category=text(args?.category||"retail",40);if(!OVERPASS_CATEGORIES[category])err("INVALID_CATEGORY",400,{allowed:Object.keys(OVERPASS_CATEGORIES)});const q=`[out:json][timeout:10];(${OVERPASS_CATEGORIES[category].replaceAll("R",String(radius)).replaceAll("LAT",String(lat)).replaceAll("LON",String(lon))});out center tags qt 200;`;const body=new URLSearchParams({data:q}).toString(),data=await jsonRequest(OVERPASS,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded",accept:"application/json","user-agent":"three-center-intelligence/1.0 bounded-commercial-spatial"},body});const elements=Array.isArray(data?.elements)?data.elements.slice(0,200):[];return{provider:"osm_overpass",operation:"nearby_features",category,radius,location:[lon,lat],osm_base:data?.osm3s?.timestamp_osm_base||null,elements,count:elements.length,policy:"fixed-query-template; serial; <=1500m; <=200 elements"}}

const WORLDMOVE_INFO={provider:"worldmove",role:"open-synthetic-human-mobility",publisher:"Tsinghua FIB Lab",access:"open-access",coverage:"1600+ cities / 179 countries",inputs:["WorldPop","POI","synthetic commuting OD"],use:"mobility prior and scenario simulation; never label as observed phone footfall",repository:"https://github.com/tsinghua-fib-lab/WorldMove",data_page:WORLDMOVE_DATA,arbitrary_url:false};
async function worldmoveRun(operation){if(operation==="source_info")return WORLDMOVE_INFO;if(operation==="download_index"){const{raw}=await request(WORLDMOVE_DATA,{headers:{accept:"text/html,application/xhtml+xml"}});const hrefs=[];const re=/href=["']([^"']+)["']/gi;let m;while((m=re.exec(raw))&&hrefs.length<100){try{const u=new URL(m[1],WORLDMOVE_DATA);if(["http:","https:"].includes(u.protocol)&&!hrefs.includes(u.href))hrefs.push(u.href)}catch{}}return{...WORLDMOVE_INFO,operation,reachable:true,links:hrefs.slice(0,100)}}err("ADAPTER_OPERATION_NOT_APPROVED",403)}
