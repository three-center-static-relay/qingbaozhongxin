...
function timeoutPromise(ms,onTimeout){return new Promise((_,rej)=>setTimeout(()=>{try{onTimeout?.()}catch{};rej(Object.assign(new Error("UPSTREAM_TIMEOUT"),{status:504}))},ms))}
async function fetchBounded(url,init={},ms=RPC_TIMEOUT_MS){const c=new AbortController();try{return await Promise.race([fetch(url,{...init,signal:c.signal}),timeoutPromise(ms,()=>c.abort())])}catch(e){if(e?.name==="AbortError")fail("UPSTREAM_TIMEOUT",504);throw e}}
