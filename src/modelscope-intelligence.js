...
async function safe(env,path){
  try{const data=await getJson(env,path),raw=JSON.stringify(data??null),bytes=new TextEncoder().encode(raw).length,itemCount=countPayload(data),nonempty=raw!=="null"&&raw!=="{}"&&raw!=="[]"&&bytes>2;return{ok:true,http_status:200,item_count:itemCount,payload_nonempty:nonempty,payload_bytes:bytes,payload_digest:nonempty?await digest(raw):null}}
...
export async function probeModelScopeIntelligence(env={}){
  ...
  if(!configured)return{ok:false,provider:"modelscope",configured:false,authenticated:false,models_ok:false,models_source_retired:true,datasets_ok:false,skills_ok:false,studios_ok:false,models_nonempty:false,datasets_nonempty:false,secret_echo:false,acceptance_state:"token-required"};
  const me=await safe(env,"/users/me");
  if(!me.ok)return{ok:false,provider:"modelscope",configured:true,authenticated:false,models_ok:false,models_source_retired:true,datasets_ok:false,skills_ok:false,studios_ok:false,models_nonempty:false,datasets_nonempty:false,http_status:me.http_status,error_class:me.error_class,secret_echo:false,acceptance_state:[401,403].includes(me.http_status)?"auth-failed":"upstream-failed"};
  const [datasets,skills,studios]=await Promise.all([safe(env,"/datasets"),safe(env,"/skills"),safe(env,"/studios")]);
  const ok=datasets.ok&&skills.ok&&studios.ok&&datasets.payload_nonempty;
  return{
    ok,provider:"modelscope",configured:true,authenticated:true,
    models_ok:false,models_count:0,models_nonempty:false,models_payload_bytes:0,models_payload_digest:null,models_source_retired:true,model_source_policy:"workers-ai,openrouter,huggingface",
    ...
    acceptance_state:ok?"live-read-non-model-resources-pass":"partial-read-api-or-empty-payload-failure",
    secret_echo:false
  };
}
