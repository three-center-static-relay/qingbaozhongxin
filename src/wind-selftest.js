...
const safeText=(v,n=160)=>String(v??"").replace(/[\r\n\t]+/g," ").trim().slice(0,n);
...
export async function runWindSelftest(env){
  ...
  try{
    ...
    return json({ok,selftest:"wind-aifin-readiness",runtime:true,business_e2e:false,protocol_e2e:true,...observed,receipt_digest,secrets_redacted:true,elapsed_ms:Date.now()-started,observed_at:new Date().toISOString()},ok?200:503);
  }catch(error){
    const code=safeText(error?.message||"WIND_SELFTEST_FAILED",80);
    const allowed=new Set(["UPSTREAM_AUTH_FAILED","UPSTREAM_RATE_LIMIT","UPSTREAM_SERVER_ERROR","UPSTREAM_UNAVAILABLE","UPSTREAM_TIMEOUT","UPSTREAM_HTTP_ERROR","UPSTREAM_JSONRPC_ERROR","UPSTREAM_EMPTY_RESPONSE","UPSTREAM_RESPONSE_FORMAT_ERROR","UPSTREAM_TOOL_ERROR"]);
    return json({ok:false,selftest:"wind-aifin-readiness",runtime:true,business_e2e:false,protocol_e2e:false,provider:"aifin_market",error:allowed.has(code)?code:"WIND_SELFTEST_FAILED",ai_called:false,secrets_redacted:true,elapsed_ms:Date.now()-started,observed_at:new Date().toISOString()},503);
  }
}
